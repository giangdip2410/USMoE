...
class DeepseekMoE(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        router_logits = hidden_states @ self.gate.weight.T #self.gate(hidden_states)
        routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
        router_logits_reshape = torch.sigmoid(router_logits) * 2.0 / self.num_experts
        # router_logits_reshape = router_logits.reshape(batch_size, sequence_length, -1)
        # router_logits_reshape = F.softmax(router_logits_reshape, dim=1, dtype=torch.float).reshape(batch_size*sequence_length, -1)
        # router_logits_reshape = router_logits_reshape / router_logits_reshape.sum(dim=1).unsqueeze(-1)
        ...
        # # select the best
        # router_logits_reshape = router_logits.reshape(batch_size, -1)
        # topk = sequence_length * self.top_k
        # _, gate_top_k_idx = torch.topk(
        #         router_logits_reshape, k=topk, dim=-1, largest=True, sorted=False
        #     )
