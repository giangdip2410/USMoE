...
from custom_gate import CustomNaiveGate_Balance, CustomNaiveGate_Balance_XMoE, CustomNaiveGate_Balance_StableMoE, CustomNaiveGate_Balance_SMoE
            ...
            for i in range(args.batch_chunk):
                ...
                if args.load_balance > 0:
                    ...
                    for name, m in model.named_modules():
                        if isinstance(m, CustomNaiveGate_Balance) or isinstance(m, CustomNaiveGate_Balance_XMoE) or isinstance(m, CustomNaiveGate_Balance_StableMoE) or isinstance(m, CustomNaiveGate_Balance_SMoE):
                            balance_loss += m.loss
                balance_loss = args.load_balance * balance_loss
                if args.fp16:
                    if args.load_balance > 0:
                        optimizer.backward(loss+balance_loss)
                    else:
                        optimizer.backward(loss)
                    #optimizer.backward(loss)
                else:
                    if args.load_balance > 0:
                        (loss+balance_loss).backward()
                    else:
                        loss.backward()
                    #loss.backward()
